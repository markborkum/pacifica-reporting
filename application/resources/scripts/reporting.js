...
var show_transaction_info = function(el, transaction_list) {
    var el = $(el);
    ...
    if (disclosure_arrow.hasClass("dc_up")) {
        ...
        if (details_container.html().length == 0) { //empty details, needs loading
            get_transaction_info(this, transaction_list);
        }
...
var get_transaction_info = function(el, transaction_list) {
    ...
    if(disclosure_arrow.hasClass("dc_down")){
...
var load_new_group_timeline_data = function(timeline_obj, object_type, group_id, start_date, end_date) {
    ...
    var getter = $.get(url, function(data) {
        ...
        var timeline = $("#object_timeline_" + group_id).highcharts($.extend(true, {}, hc_timeline_options,time_line_options));
        reload_pie_chart(timeline.highcharts());
    });
};
...
$(function() {
    ...
    Highcharts.getOptions().plotOptions.spline.colors = (function() {
        var colors = [];
        var base = Highcharts.getOptions().colors[7];   //greens
        var i;
        for (i = 0; i < 10; i += 1) {
            // Start out with a darkened base color (negative brighten), and end
            // up with a much brighter color
